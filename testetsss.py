...
def conectarServidor(servidor):
    cliente.conectar_servidor(servidor, usuario)

def criar_chat_box(root):
    # Criando a caixa de entrada para a mensagem
    mensagem_entry = ctk.CTkEntry(boxMensagem,
                    width=600,
                    placeholder_text=None,
                    corner_radius=15)
    mensagem_entry.pack(pady=20)

    # Criando o botão de enviar com um ícone
    btnEnviar = ctk.Button(root, text="Enviar", command=enviar_mensagem)
    btnEnviar.pack()

    # Ícone do botão de enviar
    enviar_icon = ctk.PhotoImage(file='img\send.png')
    btnEnviar.config(image=enviar_icon, compound=ctk.LEFT)
...
jnlLogin = ctk.CTk()
jnlLogin._set_appearance_mode("dark")
